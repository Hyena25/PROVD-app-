...
// Finished dares, mine and my friends'. Terminal statuses only — the history
// feed shows whether each one was pulled off or blown.
function mockHistoryRows(meId) {
  const me = { username: 'tester', display_name: 'Test User' };
  const maya = { username: 'maya', display_name: 'Maya' };
  const devon = { username: 'devon', display_name: 'Devon' };
  return [
    { id: 'h-1', title: 'Cold shower for 60 seconds', status: 'completed', difficulty: 'easy', points_value: 10, is_arena: false, created_at: pastIso(3), expires_at: pastIso(2), target_user_id: meId, target: me, sender: maya },
    { id: 'h-2', title: 'Text an old friend hello', status: 'completed', difficulty: 'easy', points_value: 10, is_arena: false, created_at: pastIso(7), expires_at: pastIso(6), target_user_id: meId, target: me, sender: devon },
    { id: 'h-3', title: 'Run a mile before noon', status: 'expired', difficulty: 'medium', points_value: 25, is_arena: false, created_at: pastIso(12), expires_at: pastIso(11), target_user_id: meId, target: me, sender: maya },
    { id: 'h-4', title: 'Sing in a public place', status: 'rejected', difficulty: 'hard', points_value: 50, is_arena: false, created_at: pastIso(15), expires_at: pastIso(14), target_user_id: meId, target: me, sender: devon },
    { id: 'h-5', title: 'Eat a raw lemon on camera', status: 'completed', difficulty: 'medium', points_value: 25, is_arena: false, created_at: pastIso(2), expires_at: pastIso(1), target_user_id: 'friend-1', target: maya, sender: me },
    { id: 'h-6', title: 'Do 50 burpees without stopping', status: 'missed', difficulty: 'hard', points_value: 50, is_arena: false, created_at: pastIso(5), expires_at: pastIso(4), target_user_id: 'friend-2', target: devon, sender: me },
    { id: 'h-7', title: 'Compliment three strangers', status: 'completed', difficulty: 'easy', points_value: 10, is_arena: false, created_at: pastIso(9), expires_at: pastIso(8), target_user_id: 'friend-1', target: maya, sender: devon },
    { id: 'h-8', title: 'Wear socks with sandals all day', status: 'expired', difficulty: 'easy', points_value: 10, is_arena: false, created_at: pastIso(18), expires_at: pastIso(17), target_user_id: 'friend-2', target: devon, sender: maya },
  ];
}
  ...
  async _runSelect() {
    switch (this.table) {
      ...
      case 'dares': {
        ...
        const rows = mockHistoryRows(meId);
        const wanted = this._filters.status;
        const finished = Array.isArray(wanted)
          ? rows.filter((r) => wanted.includes(r.status))
          : null;
        ...
        // History: `.in('status', [...terminal])` on my dares or a group's.
        if (finished) {
          if ('target_user_id' in this._filters) {
            return ok(
              finished.filter((r) => r.target_user_id === this._filters.target_user_id)
            );
          }
          if ('group_id' in this._filters) return ok(finished);
          return ok(finished);
        }
